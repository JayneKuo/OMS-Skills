# Product Query MVP API Mapping Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `docs/product-agent/02-product-query-api-mapping.md` so Product Agent first-version data sources, supported questions, limitations, and acceptance cases are explicit.

**Architecture:** This is a documentation-only change. The new mapping document will align Product Agent behavior with the existing `product_query`, `product_performance`, `oms_analysis`, and product skill MVP state without adding runtime integrations.

**Tech Stack:** Markdown documentation, existing Product Agent docs, existing Kiro skill docs, existing OMS Agent MCP runtime.

---

## File Structure

- Create: `docs/product-agent/02-product-query-api-mapping.md`
  - Responsibility: canonical MVP mapping for `product_query` data sources, supported intents, answer boundaries, and acceptance cases.
- Reference only: `docs/product-agent/01-落地行动方案.md`
  - Responsibility: existing implementation roadmap and current MVP status.
- Reference only: `.kiro/skills/product-query/SKILL.md`
  - Responsibility: current `product_query` skill contract and runtime status.
- Reference only: `.kiro/skills/product-diagnosis/SKILL.md`
  - Responsibility: current diagnosis expectations that consume `product_query` evidence.
- Reference only: `.kiro/skills/oms-agent/mcp_server.py`
  - Responsibility: current MCP tool entry points and confirmed runtime functions.

No tests are required because this plan creates a documentation artifact only. Verification is a content review against the approved design spec.

---

### Task 1: Create MVP API mapping document

**Files:**
- Create: `docs/product-agent/02-product-query-api-mapping.md`
- Reference: `docs/superpowers/specs/2026-05-14-product-query-mvp-mapping-design.md`
- Reference: `docs/product-agent/01-落地行动方案.md`
- Reference: `.kiro/skills/product-query/SKILL.md`

- [ ] **Step 1: Create the document with MVP positioning and runtime inputs**

Write this content at the top of `docs/product-agent/02-product-query-api-mapping.md`:
...
- [ ] **Step 2: Add the API mapping table**
...
- [ ] **Step 3: Add supported intents and answerable questions**
...
- [ ] **Step 4: Add non-commitments and degradation rules**
...
- [ ] **Step 5: Add result shape and acceptance cases**
...
- [ ] **Step 6: Review the document against the approved design**

Run:

```bash
git diff -- docs/product-agent/02-product-query-api-mapping.md
```

Expected:

- The document exists.
- It covers MVP positioning, runtime inputs, API mapping, supported intents, answerable questions, non-commitments, degradation rules, output structure, and acceptance cases.
- It does not claim sync logs, deep mappings, image sync details, market data, or competitor data are supported.

- [ ] **Step 7: Do not commit unless the user explicitly asks**

Leave the file uncommitted. Report the created file path and ask whether the user wants edits or wants to proceed to implementation/review.

```bash
git status --short docs/product-agent/02-product-query-api-mapping.md docs/superpowers/specs/2026-05-14-product-query-mvp-mapping-design.md docs/superpowers/plans/2026-05-14-product-query-mvp-api-mapping.md
```

Expected: the new/modified documentation files are visible in working tree status.

---

## Self-Review

- Spec coverage: The plan creates the requested mapping document and includes every approved section: scope, runtime inputs, mapping table, supported questions, non-commitments, degradation behavior, output shape, and acceptance cases.
- Placeholder scan: The plan contains no `TBD`, `TODO`, unspecified edge handling, or references to undefined implementation steps.
- Type consistency: The intent names and output fields match the Product Query skill contract and approved spec.
