...
name: order_query
description: >
  OMS 订单全景查询工具。当用户需要查询订单状态、shipment、库存、仓库、分仓结果、
  规则命中、异常、暂停履约、关键事件时，通过调用 OMS API 获取真实数据，
  归一化状态，输出查询级解释。
  关键词：订单查询、order query、订单状态、拆单结果、dispatch log、履约状态、
  shipment、库存、仓库、异常、Hold、暂停履约、规则命中。
license: MIT
metadata:
...
迭代记录:
  - 版本: v4.0
    日期: 2026-04-08
    变更: 与需求规格 v5.0 对齐；修复 8 项不一致；新增查询编排策略；
          更新全部 API 为前端扫描确认的真实路径；删除过时的降级描述
  - 版本: v3.0
    日期: 2026-04-08
