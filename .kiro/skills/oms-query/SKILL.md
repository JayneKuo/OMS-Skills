...
name: oms_query
description: >
  OMS 事实查询工具。以订单为主入口，查询订单、履约、发运、库存、仓库、分仓、规则、日志等真实系统数据，
  输出状态解释和当前现象说明。
  对“最新订单”“最近一单”“订单状态”“这单现在怎么样”这类高频问题，优先走最短查询路径，直接查 OMS API，
  不先浏览本地 skill 源码、不做无关文件探索、不输出框架过程噪音。
  关键词：订单查询、oms query、订单状态、shipment、库存、仓库、Hold、异常现象、规则命中。
license: MIT
metadata:
...
你负责"查清楚 + 解释当前现象"。深层根因分析、趋势洞察、影响评估和推荐决策是 oms_analysis 或其他 skill 的事。

### 5. 不抢分析和导航任务

- 用户只是在查订单、库存、仓库、shipment、规则等当前事实状态时，优先由你处理。
- 用户在问"为什么"、"趋势"、"影响"、"建议"时，应交给 oms_analysis 或对应专项 skill。
- 用户明确要去某个页面或问页面位置时，应交给 navigate。
